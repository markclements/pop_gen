...
server <- function(input, output) {
	...
	observeEvent(input$update, 
							 ...
							 if(rv$allele1==1 & rv$allele2==1 & runif(1,0,1)<=input$RFBB) {
							 rv$df2[rv$index,3]<-rv$allele1
							 rv$df2[rv$index,4]<-rv$allele2
							 rv$index<-rv$index+1
							 }
							 
							 if(rv$allele1==1 & rv$allele2==2  & runif(1,0,1)<=input$RFRB) {
							 	rv$df2[rv$index,3]<-rv$allele1
							 	rv$df2[rv$index,4]<-rv$allele2
							 	rv$index<-rv$index+1
							 }
							 
							 if(rv$allele1==2 & rv$allele2==1  & runif(1,0,1)<=input$RFRB) {
							 	rv$df2[rv$index,3]<-rv$allele1
							 	rv$df2[rv$index,4]<-rv$allele2
							 	rv$index<-rv$index+1
							 }
							 
							 if(rv$allele1==2 & rv$allele2==2 & runif(1,0,1)<=input$RFRR) {
							 	rv$df2[rv$index,3]<-rv$allele1
							 	rv$df2[rv$index,4]<-rv$allele2
							 	rv$index<-rv$index+1
							 }
	...
	output$nextgen <- renderUI({
		if (rv$index > input$xaxis*input$yaxis) {
			actionButton("nextgen", "Next Generation") 
		}
	})
